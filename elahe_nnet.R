...
housing = read.csv("C:/Users/rezas/Desktop/white_wine.csv")
...
colnames(housing)= c("fixed_acidity","volatile_acidity","citric_acid","residual_sugar","chlorides","free_sulfur_dioxide","total_sulfur_dioxide","density","pH","sulphates","alcohol","quality")
...
system.time(nn4 <- neuralnet(f, data = train, hidden = 3, algorithm = "sag", learningrate.limit = c(0.1,1), stepmax = 100000, err.fct = "sse", lifesign = "full"))
plot(nn4)
predY4 <- compute(nn4,test[,-12])
cor(predY4$net.result, test[12])
