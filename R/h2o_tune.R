...
h2o.init(
  ...
  max_mem_size = "6G")    ## specify the memory size for the H2O cloud
...
splits <- h2o.splitFrame(
  housing,           ##  splitting the H2O frame we read above
  c(0.6,0.2),   ##  create splits of 60% and 20%; 
  ##  H2O will create one more split of 1-(sum of these parameters)
  ##  so we will get 0.6 / 0.2 / 1 - (0.6+0.2) = 0.6/0.2/0.2
  seed=1981)    ##  setting a seed will ensure reproducible results (not R's seed)
...
train[1:14,]   ## rows 1-14, all columns

## run our first predictive model
rf1 <- h2o.randomForest(         ## h2o.randomForest function
  training_frame = train,        ## the H2O frame for training
  validation_frame = valid,      ## the H2O frame for validation (not required)
  x=2:14,                        ## the predictor columns, by column index
  y=1,                          ## the target index (what we are predicting)
  model_id = "rf_covType_v1",    ## name the model in H2O
  ##   not required, but helps use Flow
  ntrees = 5000,                  ## use a maximum of 200 trees to create the
  nfolds = 10,
  ##  random forest model. The default is 50.
  ##  I have increased it because I will let 
  ##  the early stopping criteria decide when
  ##  the random forest is sufficiently accurate
  stopping_rounds = 0,           ## Stop fitting new trees when the 2-tree
...
###############################################################################
summary(rf1)  
pred_rf1 <- h2o.predict(rf1,newdata = test)
h2o.cor(pred_rf1,test[,1])
...
###############################################################################
summary(rf2)
pred_rf2 <- h2o.predict(rf2,newdata = test)
h2o.cor(pred_rf2,test[,1])
